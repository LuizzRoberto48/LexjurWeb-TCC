...
export class LawyerPermissionFormComponent {
  ...
  update(obj: CreateUserPermission) {
    ...
    this.userPermission.update(obj).subscribe({
      next: () => {
        this.notification.success('Permissão alterada com sucesso');
        this.toBack()
      },
    });
  }
  ...
  toBack() {
    this._router.navigateByUrl('lawyers/permissions')
  }
  ...
  formToObj() {
    const { permissionFeatures, ...obj } = this.form.getRawValue();
    const transformed = {
      name: obj.name,
  ...
  selected(event: any, i: number): void {
    this.addRole(event.option.value, i);
  }
