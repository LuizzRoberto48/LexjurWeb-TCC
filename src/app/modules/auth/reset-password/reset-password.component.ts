...
import { CreateUserPassword } from 'app/core/auth/models/create-user-password';
...
export class AuthResetPasswordComponent implements OnInit {
  ...
  tokenExpired(expToken:number) {
    if(this._authService.isTokenExpired(expToken)) {
      ...
      this.alert = {
        type: 'error',
        message: 'O token foi expirado. Envie outra solicitação de mudança de senha'
      };
    }
  }
  ...
  resetPassword(): void {
    ...
    this._authService.resendPassword(createUserObj).pipe(
      finalize(() => {
